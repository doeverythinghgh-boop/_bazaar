  ...
  try {
    ...
    Swal.fire({
      ...
      html: `
                <div class="swal-custom-actions" style="display: flex; flex-wrap: wrap; justify-content: center; gap: 8px; margin-top: 20px;">
                    ...
                    <button id="swal-factory-reset-btn" class="swal2-styled" style="background-color: #dc3545; font-size: 0.85em; padding: 0.5em 1.2em; color: white; width: 100%; border-radius: 8px; margin-top: 10px;">
                        <i class="fas fa-trash-alt"></i> ${window.langu("dash_btn_factory_reset")}
                    </button>
                </div>
            `,
      ...
      if (result.isConfirmed) {
        ...
        window.location.href = "/pages/ADMIN/mainAdvertises.html";
      }
    });
    ...
    document
      ...
      .addEventListener("click", () => {
        window.location.href = "/notification/page/admainNotificationsetting.html";
        Swal.close();
      });

    // ✅ ربط زر المنتجات المعلقة (فتح مستقل)
    document.getElementById("swal-pending-products-btn").addEventListener("click", () => {
      window.location.href = "/pages/ADMIN/pendingProducts.html";
      Swal.close();
    });

    // ✅ ربط زر إعادة ضبط المصنع
    document.getElementById("swal-factory-reset-btn").addEventListener("click", () => {
      // [Safety check] Prepare confirmation texts
      const title = (window.appTranslations && window.appTranslations.factory_reset_confirm) ? window.appTranslations.factory_reset_confirm.title[localStorage.getItem('app_language') || 'ar'] : "هل أنت متأكد؟";
      const text = (window.appTranslations && window.appTranslations.factory_reset_confirm) ? window.appTranslations.factory_reset_confirm.text[localStorage.getItem('app_language') || 'ar'] : "سيتم مسح كافة البيانات تماماً، لا يمكن التراجع!";
      const confirmBtn = (window.appTranslations && window.appTranslations.factory_reset_confirm) ? window.appTranslations.factory_reset_confirm.confirm_btn[localStorage.getItem('app_language') || 'ar'] : "نعم، امسح الكل";
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          // Execute Hard Reset
          if (typeof appHardReset === 'function') {
            appHardReset();
          } else {
            console.error("appHardReset is not defined");
            location.reload();
          }
        }
      });
...
function updateViewForLoggedInUser() {
  ...
  // [Step 2.1] Update User Avatar
  const avatarImg = document.getElementById("dash-user-avatar");
  const avatarPlaceholder = document.getElementById("dash-user-avatar-placeholder");
  const avatarContainer = document.getElementById("dash-user-avatar-container");

  if (avatarImg && avatarContainer) {
    if (user.user_image) {
      // If image exists, construct full URL and show it
      // Assuming getPublicR2FileUrl is available globally (it is in tools/cloudFileManager.js or similar)
      if (typeof getPublicR2FileUrl === 'function') {
        avatarImg.src = getPublicR2FileUrl(user.user_image);
    ...
    // Make avatar clickable to edit profile (UX improvement)
    if (SessionManager.isGuest()) {
      avatarContainer.style.cursor = "default";
      avatarContainer.onclick = function () {
        // Guest user: Do nothing (No profile access, no alert)
        return;
      };
    } else {
        ...
        <div class="custom-settings-modal">
           ...
           <div id="settings_modal_content" class="settings-modal-content" style="text-align: inherit; direction: inherit;">
            ...
            <div id="settings_list_options" style="display: flex; flex-direction: column; gap: 8px;">
               ...
               <!-- Profile Settings (Smart Logic) -->
               ${(() => {
          const isGuest = SessionManager.isGuest();
          if (isGuest) {
            return `
                     <div class="settings-list-item disabled" style="opacity: 0.5; cursor: not-allowed;">
                        <span>
                           <i class="fas fa-user-lock" style="color: #ccc;"></i> ${window.langu("dash_profile_tab")} <small>(${window.langu("guest_mode_label") || "زائر"})</small>
                        </span>
                     </div>
                    `;
          } else {
            ...
            return `
                     <div class="settings-list-item disabled" style="opacity: 0.5; cursor: not-allowed; justify-content: space-between;">
                        <span style="display: flex; align-items: center; gap: 8px;">
                           <i class="fas fa-boxes" style="color: #ccc; width: 20px;"></i> 
                           ${window.langu("dash_fetch_orders_title")}
                        </span>
                        <small>(${window.langu("guest_mode_label") || "زائر"})</small>
                     </div>
                    `;
          }
          ...
          if (isAdmin || isImpersonating) {
            ...
            const isSuperAdmin = (user && window.SUPER_ADMIN_KEY && user.user_key === window.SUPER_ADMIN_KEY);
            const adminLabel = isSuperAdmin ? window.langu('dash_opt_super_admin') : window.langu('dash_opt_admin');
            ...
            return `
                             <div class="settings-list-item" style="cursor: default; opacity: 0.8;">
                                <span style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                                   ...
                                   <span style="font-size: 0.9em; font-weight: bold; color: var(--danger-color);">${adminLabel} <i class="fas fa-lock" style="font-size: 0.8em;"></i></span>
                                </span>
                             </div>
                           `;
          } else {
          ...
          if (isGuest) {
            return `
                     <div class="settings-list-item disabled" style="opacity: 0.5; cursor: not-allowed; justify-content: space-between;">
                        <span>
                           <i class="fas fa-bell-slash" style="color: #ccc; width: 20px;"></i> ${window.langu("dash_notifications_tab")}
                        </span>
                        <small>(${window.langu("guest_mode_label") || "زائر"})</small>
                     </div>
                    `;
            ...
            return `
               ...
               <div id="settings_list_support" onclick="window.location.href = '/pages/contact.html'; closeSettingsModal();"
                    class="settings-list-item">
                  <span>
...
// [Final Step] Initialize (handled by standalone page DOMContentLoaded)
