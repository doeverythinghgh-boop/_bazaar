...
const SessionManager = {
    ...
    init: () => {
        try {
            ...
            // Sync UI
            if (typeof setUserNameInIndexBar === 'function' && document.getElementById("index-login-text")) {
    ...
    login: async (user, redirect = true) => {
        ...
        // Ensure user_image exists (even as null if undefined) to avoid property access issues
        user.user_image = user.user_image || null;

        // Ensure Business Fields exist
        user.business_name = user.business_name || "";
        user.business_category = user.business_category || "";
        user.business_bio = user.business_bio || "";
        user.business_whatsapp = user.business_whatsapp || "";
