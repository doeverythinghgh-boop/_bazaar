...
async function mainLoader(
    ...
    try {
        ...
        if (!reload && currentLoadedUrl === pageUrl) {
            ...
            // 1. Manage registry (hides others, moves this to top)
            ...
            if (container) {
                ...
                // 3. Re-trigger Entrance Animation for smooth transition
                container.classList.remove('animate-fade-up');
                void container.offsetWidth; // Force reflow
                container.classList.add('animate-fade-up');
            }

            // 4. Ensure translations are up to date
            if (typeof window.applyAppTranslations === 'function') {
                await window.applyAppTranslations();
            }

            // 5. Final output log for Smart Loading
            console.log(
                `%c✔✔✔ [SmartLoader] تم إظهار الحاوية من الذاكرة بنجاح ✔✔✔\n` +
