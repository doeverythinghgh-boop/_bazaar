...
/**
 * @description Global set of selected products for search admin view.
 * Syncs with 'selected_search_products.json' via initSelectedProductsModule.
 * @type {Set<string>}
 * @global
 */
...
function loadProductView(productData, options = {}) {
  ...
  // Ensure data is mapped using the unified mapper
  const mappedData = (typeof window.mapProductData === 'function') 
    ? window.mapProductData(productData) 
    : productData;

  if (!mappedData) {
      console.error('[ProductView] فشل في معالجة بيانات المنتج أو البيانات غير صالحة.');
      return;
  }
  ...
  // Determine page path (Absolute for standalone)
  const pagePath = isService
    ? "/pages/productView2/productView2.html"
    : "/pages/productView/productView.html";

  console.log(`[ProductView] التنقل إلى صفحة مستقلة: ${pagePath}`);
...
function loadProductForm(options = {}) {
  ...
  // Navigate directly to standalone page
  window.location.href = pagePath;
}
