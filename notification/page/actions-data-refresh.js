...
export const ActionsDataRefresh = {
    /**
     * @description Load notifications from LocalDB.
     */
    async loadNotifications() {
        this.setState({ isLoading: true, hasError: false });
        try {
            if (typeof window.initDB === 'function') await window.initDB();
            
            let notifications = [];
            if (typeof window.getNotificationLogs === 'function') {
                notifications = await window.getNotificationLogs('all', 1000);
            }
    ...
    async deleteNotification(id, element) {
        try {
            ...
            if (result.isConfirmed) {
                if (typeof window.deleteNotificationFromDB === 'function') await window.deleteNotificationFromDB(id);
    ...
    async markAllAsRead(silent = false) {
        console.log(`[BADGE_DIAG] markAllAsRead invoked. silent=${silent} currentLocalUnread=${this.state.notifications.filter(n => n.status === 'unread').length}`);
        if (!silent) {
            const result = await Swal.fire({
        ...
        try {
            if (typeof window.markAllNotificationsAsReadInDB === 'function') await window.markAllNotificationsAsReadInDB();
            ...
            this.updateStats(this.state.notifications);
            console.log('[BADGE_DIAG] markAllAsRead finished. local unread forced to 0 and stats refreshed.');
        } catch (error) {
...
// Hybrid bridge
Object.assign(NotificationPage, ActionsDataRefresh);
window.NotificationPage = NotificationPage;

console.log("[ESM Load] actions-data-refresh.js: Hybrid bridge established.");
