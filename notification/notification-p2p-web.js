...
export const WebP2PNotification = (() => {
    ...
    const isNativeAndroidRuntime = !!window.Android ||
        (window.BridgeManager && typeof window.BridgeManager.isAndroid === 'function' && window.BridgeManager.isAndroid()) ||
        window.location.hostname === 'appassets.androidplatform.net';

    if (isNativeAndroidRuntime) {
        console.log('[Web P2P] Android Bridge detected. Disabling redundant Web P2P module.');
        return {
    ...
    async function getAccessToken() {
        ...
        try {
            ...
            if (!adminKey) {
                throw new Error('FCM_ADMIN_SDK_KEY is not defined. Ensure notification-credentials.js is loaded.');
            }

            const firebaseRuntime = typeof window.getBazaarFirebaseConfig === 'function'
                ? window.getBazaarFirebaseConfig()
                : {};
            const key = firebaseRuntime.adminServiceAccount || adminKey;
            ...
            const sJWT = window.KJUR.jws.JWS.sign("RS256", sHeader, sPayload, privateKey);
    ...
    async function sendDirect(token, title, body) {
        try {
            ...
            if (response.ok) {
                ...
                const isUnregistered = result?.error?.message === 'Requested entity was not found.' ||
                                     result?.error?.status === 'NOT_FOUND' ||
                                     result?.error?.details?.some(d => d.errorCode === 'UNREGISTERED');

                if (isUnregistered && typeof window.deleteTokenFromServer === 'function') {
                    console.warn('[Web P2P] Token is unregistered. Cleaning it from Turso.', token.substring(0, 12) + '...');
                ...
                return { error: result, tokenStatus: isUnregistered ? 'unregistered' : 'failed' };
            }
        } catch (error) {
