 ...
 * @const
 */
var DB_VERSION = 5; // Bumped to 5 to force recreation of stores and sync with order-db-manager.js
/**
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    try {
      ...
      request.onupgradeneeded = (event) => {
        ...
        // ✅ إضافة: التأكد من إنشاء مخزن الطلبات أيضاً لضمان التناسق
        if (!tempDb.objectStoreNames.contains('ordersStore')) {
          console.log('[DB] جاري إنشاء مخزن الطلبات: ordersStore');
          const oStore = tempDb.createObjectStore('ordersStore', { keyPath: 'order_key' });
          oStore.createIndex('created_at', 'created_at', { unique: false });
          oStore.createIndex('user_key', 'user_key', { unique: false });
          oStore.createIndex('role_context', 'role_context', { unique: false });
          oStore.createIndex('order_status', 'order_status', { unique: false });
        }
      };
