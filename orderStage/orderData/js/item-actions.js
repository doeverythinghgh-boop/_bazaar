...
window.OrderData_ItemActions = {
     ...
     * 1. This function performs a LOCAL ONLY update to LocalDB.
     *    - It does NOT sync with the server immediately to allow the merchant to batch updates.
     *    - Sync happens when the merchant confirms the "Review Stage" (Step 1 -> 2).
     *
     * 2. [LOCKING MECHANISM]:
    ...
    updateAvailability: async function (productKey, isAvailable) {
        ...
        // 1. Get current order data from LocalDB
        ...
        if (currentStep >= 2) {
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    icon: 'error',
                    title: 'عذراً',
                    text: 'لا يمكن تعديل حالة الخدمات بعد اعتماد المراجعة.',
