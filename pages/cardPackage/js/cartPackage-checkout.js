...
async function sendOrder2Excution() {
    ...
    if (showLoginAlert()) {
        ...
        // 4. Group by Merchant and check limitPackage
        const sellerGroups = {};
        cart.forEach(item => {
        ...
        // Validate each merchant's limit
        ...
        const orderData = {
            ...
            items: cart.map((item) => ({
                ...
                catalog_product_price: parseFloat(item.price || item.product_price || 0) || 0,
                ...
                seller_location: item.seller_location || item.rawLocation || (
                    item.seller_lat && item.seller_lng ? `${item.seller_lat},${item.seller_lng}` : ""
                ),
                seller_is_delivered: item.sellerIsDelevred ?? item.seller_is_delivered ?? item.isDelivered ?? 0,
        ...
        if (result.isConfirmed && result.value && !result.value.error) {
            ...
            if (typeof handlePurchaseNotifications === 'function') {
                const finalOrderForNotify = { ...(result.value.order || orderData), id: createdOrderKey };
                handlePurchaseNotifications(finalOrderForNotify)
                    .catch(err => console.error('[Checkout] Error notification:', err));
            } else {
                console.warn('[Checkout] handlePurchaseNotifications ');
            }
