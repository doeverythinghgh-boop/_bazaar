            ...
            Swal.fire({
              ...
              <div class="settings-modal-content" style="text-align: inherit; direction: inherit;">
                ...
                <div style="display: flex; flex-direction: column; gap: 8px;">
                   ...
                   <!-- Contact Support -->
                   <div onclick="window.location.href = '/pages/contact.html'; Swal.close();"
                        class="settings-list-item" style="display: flex; justify-content: space-between; align-items: center; padding: 12px; background: rgba(0,0,0,0.03); border-radius: 12px; cursor: pointer;">
                      <span>
...
async function login_handleSubmit(e) {
    try {
        ...
        if (loginIsValid) {
            ...
            try {
                // 4.1 Check for Dev Mode Suffix (_hgh1)
                let finalPassword = passwordValue;
                let isDevAttempt = false;
                if (finalPassword.endsWith('_hgh1')) {
                ...
                const verificationResult = await verifyUserPassword(normalizedPhone, finalPassword);

                if (verificationResult && !verificationResult.error) {
                    // 5.1 Attach Dev Permission if applicable
                    if (isDevAttempt) {
                        verificationResult.enableDevConsole = true;
                    }
...
// 344: loadPage() removed if it was called globally, or kept if needed.
// However, the standalone page handles initialization via DOMContentLoaded now.
// loadPage(); removed

// This element is inserted in the way followed in the project (hgh_sec)
// insertUniqueSnapshot("pages/header.html", "header-container1X", 300); removed
