...
function EDIT_createPreviewItem(state, existingImageUrl = null) {
    ...
    if (state.status === 'pending') meta.classList.add('is-pending');

    // Track element in state
    state._el = wrapper;
    ...
    if (existingImageUrl) {
        ...
        // [OPTIMIZATION] Use Object URL instead of Base64 to save memory
        const objectUrl = URL.createObjectURL(state.file);
        img.src = objectUrl;
        state._objectUrl = objectUrl; // Cache to revoke later
    }
...
function EDIT_attachEventListeners() {
    ...
    const editBtnDiscardTop = document.getElementById('edit_btn_discard_top');
    ...
    if (editBtnDiscard) editBtnDiscard.onclick = handleDiscard;
    if (editBtnDiscardTop) editBtnDiscardTop.onclick = handleDiscard;
...
// [PREVENTION] Warn user before leaving if images are still processing
window.addEventListener('beforeunload', (e) => {
    if (typeof EDIT_images !== 'undefined' && EDIT_images.some(img => img.status === 'compressing' || img.status === 'pending')) {
        e.preventDefault();
