...
function EDIT_initSubmitLogic() {
    ...
    form.onsubmit = async (e) => {
        ...
        if (EDIT_images.length === 0) {
            ...
            // [SAFETY CHECK] Prevent saving while images are still being processed
            const processing = EDIT_images.some(img => img.status === 'compressing' || img.status === 'pending');
            if (processing) {
                EDIT_showError(uploaderEl, window.langu('edit_err_img_processing')); // Ensure this key exists or use fallback
                isValid = false;
            }
        }
