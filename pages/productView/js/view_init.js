...
/**
 * @function productView_initPage
 * @description Manual entry point to initialize the page after translations are ready.
 */
window.productView_initPage = async function () {
    ...
    try {
        ...
        // Force mapping for consistency, especially when recovered from storage
        if (productData && typeof window.mapProductData === 'function') {
            const mapped = window.mapProductData(productData);
            if (mapped) productData = mapped;
        }

        // Check if data is "lite" (missing detailed fields like description or user_key)
        if (productData && productData.product_key && (!productData.description || !productData.sellerName || productData.sellerName === "بائع غير معروف")) {
            console.log(`[ProductView] Data appears incomplete for key: ${productData.product_key}. Fetching full details...`);
