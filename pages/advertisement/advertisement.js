 ...
 *   Fetches selected products JSON with caching (1 hour).
 * @global
 */
window.initSelectedProductsModule = async function (forceRefresh = false) {
    ...
    if (!forceRefresh && lastCheck && (Date.now() - lastCheck < CHECK_INTERVAL) && cachedData) {
        console.log('%c[SelectedProducts] تحميل المنتجات المختارة من الذاكرة (ساعة واحدة).', 'color: green;');
        try {
    ...
    // 2. Fetch from Server
    console.log('%c[SelectedProducts] جلب المنتجات المختارة من السيرفر...', 'color: orange;');
    try {
        if (typeof getPublicR2FileUrl !== 'function') return;
        ...
        if (res.ok) {
            ...
            if (Array.isArray(list)) {
                ...
                console.log(`[SelectedProducts] تم تحديث القائمة: ${list.length} منتج.`);
            }
        } else {
            console.warn("[SelectedProducts] لم يتم العثور على الملف (404) أو خطأ.");
            // If 404, valid state is empty list? Yes.
            if (res.status === 404) {
                window.selectedSearchProductsSet = new Set();
                localStorage.setItem(CACHE_KEY_DATA, '[]');
                localStorage.setItem(CACHE_KEY_TIME, Date.now());
            }
        }
    } catch (e) {
