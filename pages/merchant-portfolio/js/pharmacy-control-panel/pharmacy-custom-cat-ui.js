...
(function () {
    ...
    function pharmacyRenderCustomCategoryTree(categories, standardCategories = []) {
        ...
        if ((!categories || categories.length === 0) && (!standardCategories || standardCategories.length === 0)) {
            listContainer.innerHTML = `<span id="custom-cats-empty-text">${window.pharmacyL('no_custom_cats')}</span>`;
        ...
        allParentGroups.forEach(main => {
            ...
            const badge = main.isStandard
                ? `<span style="font-size:0.65rem; background:#6c757d; color:#fff; padding:2px 6px; border-radius:10px; margin-inline-start:5px;">${window.pharmacyL('catalog_badge')}</span>`
                : (main.isPending ? `<span style="font-size:0.65rem; background:#ffc107; color:#000; padding:2px 6px; border-radius:10px; margin-inline-start:5px;">${window.pharmacyL('draft_badge')}</span>` : '');
            ...
            if (relatedSubs.length === 0 && !main.isStandard) {
                subsArea.innerHTML = `<span style="font-size:0.75rem; color:#888; font-style:italic;">${window.pharmacyL('no_sub_sections_yet')}</span>`;
            } else {
        ...
        if (parentSelect) {
            ...
            if (standardCategories && standardCategories.length > 0) {
                ...
                stdGroup.label = window.pharmacyL('std_sections_group');
            ...
            if (mainCats.length > 0) {
                ...
                custGroup.label = window.pharmacyL('my_custom_sections_group');
                mainCats.forEach(m => {
                    const opt = document.createElement('option');
                    opt.value = m.id;
