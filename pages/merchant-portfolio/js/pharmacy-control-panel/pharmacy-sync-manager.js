...
async function pharmacyLoadCatalogData(preferenceState, userKey) {
    ...
    try {
        ...
        const data = context.mergedCategories || [];
...
function pharmacySetupPreferenceSaving(preferenceState, userKey) {
    ...
    container.addEventListener('change', (e) => {
        ...
        if (toggle.tagName === 'INPUT' && toggle.type === 'checkbox') {
            const idVal = toggle.dataset.id;
            const id = isNaN(Number(idVal)) ? String(idVal) : Number(idVal);
            ...
            if (type === 'main') {
                ...
                if (badge) {
                     ...
                     if (mainObj) {
                         const count = (mainObj.sub || []).filter(s => {
                             const subId = isNaN(Number(s.id)) ? String(s.id) : Number(s.id);
                             return preferenceState.hidden_sub_categories.has(subId);
                         }).length;
                         if (count > 0) {
            ...
            if (isActuallyDirty) {
                ...
                LocalDBStorage.setItem(localKey, JSON.stringify(localPayload));
            } else {
                LocalDBStorage.removeItem(localKey);
            }
...
async function pharmacyPerformSave(preferenceState, userKey) {
    ...
    if (dismiss === Swal.DismissReason.cancel) {
        ...
        LocalDBStorage.removeItem(`pharmacy_pending_prefs_${userKey}`);
    ...
    try {
        ...
        Swal.fire({
            ...
            if (window.PortfolioErrorUtils?.log) {
                window.PortfolioErrorUtils.log("PharmacySyncManager", "Failed to save pharmacy preferences.", error);
            } else {
                console.error("Pharmacy: Failed to save preferences", error);
            }
