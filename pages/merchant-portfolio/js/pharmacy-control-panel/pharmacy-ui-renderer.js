...
function pharmacyRenderCatalog(data, preferenceState) {
    ...
    data.forEach(main => {
        ...
        const hiddenSubsCount = (main.sub || []).filter(sub => preferenceState.hidden_sub_categories.has(parseId(sub.id))).length;
        ...
        const isMainChecked = !preferenceState.hidden_main_categories.has(parseId(main.id)) ? 'checked' : '';
        ...
        if (Array.isArray(main.sub) && main.sub.length > 0) {
            main.sub.forEach(sub => {
                ...
                const isSubChecked = !preferenceState.hidden_sub_categories.has(parseId(sub.id)) ? 'checked' : '';
        ...
        card.innerHTML = `
            <div class="catalog-card-header" id="catalog-card-header-${main.id}" style="cursor:pointer;">
                <span id="main-cat-title-wrapper-${main.id}">
                    ...
                    <span id="main-cat-title-${main.id}" class="main-cat-title-text">${mainTitle}</span>
                    ${hiddenBadgeHtml}
                </span>
