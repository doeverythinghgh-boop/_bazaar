...
async function portfolioHandleRate(targetUser) {
    ...
    let settings = {};
    try {
        settings = typeof targetUser.settings === 'string' ? JSON.parse(targetUser.settings || '{}') : (targetUser.settings || {});
    } catch (e) {
        settings = {};
    }
    ...
    if (typeof SessionManager !== 'undefined' && !SessionManager.getUser()) {
        Swal.fire({
            ...
            text: L('port_rate_login_required_text', 'يجب تسجيل الدخول لتقييم التاجر'),
            icon: 'warning',
            buttonsStyling: false,
            customClass: {
                popup: 'swal-modern-mini-popup',
    ...
    const { value: formValues } = await Swal.fire({
        title: L('port_rate_dialog_title', 'تقييم التاجر'),
        html: `
