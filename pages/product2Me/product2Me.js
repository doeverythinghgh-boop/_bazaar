(function () {
  ...
  async function loadProducts(filters = {}) {
    ...
    baseParams.append("user_key", currentUser.user_key || ""); // Filter by current user
          ...
          if (product.ImageName) {
            ...
            if (imageNames.length > 0) {
              await Promise.all(imageNames.map(name =>
                window.deleteFile2cf(name).catch(err => console.error(`[Delete] فشل حذف الصورة ${name}:`, err))
              ));
            }
          }
  ...
  /**
   * @function initializeProduct2Me
   * @description Initializes the page data and filters.
   */
  ...
  // Initialize immediately if not in standalone (fallback for mainLoader)
  if (!document.getElementById('product-2me-page')) {
    initializeProduct2Me();
  }
    ...
    p2m_btnBack.onclick = function () {
      try {
        if (window.containerGoBack) {
          ...
          history.back(); // Fallback for standalone mode
        }
      } catch (error) {
